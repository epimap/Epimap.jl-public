...
@model function rmap_debiased(
    ...
    # Likelihood.
    @submodel logitπ = DebiasedLikelihood(logitπ, σ_debias, populations, X, D, num_cond, T; skip_weeks_observe)
...
@model function DebiasedLikelihood(
    logitπ, σ_debias, populations, X, D, num_cond, ::Type{T}=Float64;
    skip_weeks_observe=0
) where {T}
    # Convolution.
    # Clamp the values to avoid numerical issues during sampling from the prior.
